...
### Fixed
- **Eagerly compile JSON schemas in `ConmanCache.register()` instead of lazily in `RequestValidator` under request load.** `MockConfig.RequestValidation` gains a `precompileBodySchemaIfPresent()` method (idempotent); `ConmanCache.register` calls it for each loaded mock. Eliminates the small race where two concurrent first-time requests for the same mock could each compile the same JSON schema and write to `bodySchemaInternal`. Last-write-wins meant the existing behaviour was *not incorrect* but wasn't provably correct under all schedulings either, and wasted CPU on cold start. A misconfigured schema now fails at mock-load time with a clear `RuntimeException` instead of on the first request that hits it. `RequestValidator`'s lazy-compile code path is kept as a defensive fallback for callers constructing `MockConfig` directly (tests, custom integrations).
...
### Changed
- **`POST /conman/admin/register` now uses reactive `FilePart` instead of servlet-API `MultipartFile`.** The HTTP contract is unchanged — clients still send `multipart/form-data` with a `registrationFile` part and an optional `tenantId` part. Server-side the controller method signature changed from `Map<String, String> register(String, MultipartFile)` to `Mono<Map<String, String>> register(String, FilePart)`, accumulating bytes via `DataBufferUtils.join(filePart.content())`. This removes the servlet compatibility shim and lets the WebFlux multipart codec actually stream the upload. The 1 MB controller-level size check is preserved as defence-in-depth on top of the framework-level caps from `application.yml`.
...
### Added
- **JWT claims → outbound headers**: new `JwtClaimsToHeadersWebFilter` propagates selected JWT claims as request headers downstream, so upstream microservices don't have to re-parse the token. Driven by a new `gateway.security.oauth2.claim-headers` list. Each rule is `header` + `claim`, with optional `extract` (named-group regex), `named-group`, `default`, and `if-previous-blank`. Regex patterns are compiled eagerly at startup so a malformed pattern fails the boot rather than the first matching request. Empty rule list → filter is a pass-through with zero per-request cost. Non-JWT requests (anonymous, basic auth) are pass-throughs. See `docs/security.md#jwt-claims--outbound-headers` for the full recipe and `docs/extension-points.md` for the recipe to replace the bundled filter with a custom one when conditional / lookup logic is needed.
- **CycloneDX SBOM** generated and deployed alongside each Maven Central artifact. `cyclonedx-maven-plugin` runs in the `package` phase; produces `${artifactId}-${version}-cyclonedx.json` + `.xml` (schema 1.6) per module covering compile / runtime / provided scopes (test scope excluded). Downstream consumers can ingest it into Dependency-Track, FOSSA, Snyk, or GitHub's dependency graph to answer supply-chain questions without re-deriving the transitive tree.
- **JaCoCo coverage gate** in the parent pom. The `check` execution binds to `verify` and fails the build if module-level line coverage drops below `40%` or branch coverage below `35%`. Conservative thresholds chosen to catch a real regression without failing on today's `gateway-core` baseline; tighten over time as tests grow. **`gateway-app` and `gateway-starter` are exempt** (`<jacoco.skip>true</jacoco.skip>` in each child pom). `gateway-app` is a thin Spring Boot main with no business logic worth gating; `gateway-starter` is pure `@ConditionalOn*` autoconfig glue whose interesting branches are exercised by downstream integration tests rather than units. Threshold values exposed as `${jacoco.coverage.line.minimum}` and `${jacoco.coverage.branch.minimum}` properties so downstream forks can tune without editing plugin config.
