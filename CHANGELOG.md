...
- **`@com.github.ifrugal.gateway.core.annotation.Internal` marker annotation** (revives the `core.annotation` package that the `@EnableGatewayToolkit` removal had emptied). Applied to `BodyCaptureRequest`, `BodyCaptureResponse`, `BodyCaptureExchange`, `LoggingAndCachingWebFilter`, and `RequestMatcher` — all classes that have to be `public` for Spring's reflective wiring or for cross-package use within this library, but that are not part of the supported public API. `@Retention(CLASS)` so the marker is detectable by API-audit tools (`revapi`, `apilyzer`, IntelliJ structural search) without leaking into runtime reflection. The contract — "may change in any release including PATCH; replace via `@ConditionalOnMissingBean` instead of subclass" — is documented in `docs/versioning.md` and `docs/extension-points.md`.
